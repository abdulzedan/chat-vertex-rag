...
const FileUploader: React.FC<FileUploaderProps> = ({
      ...
      try {
        ...
        if (error instanceof Error) {
          if (error.name === 'AbortError') {
            setUploadError(
              'Upload timed out. Large files may still be processing in the background.'
            );
          } else {
  ...
  const handleDriveUrl = useCallback(async () => {
    ...
    const driveUrlPattern =
      /^https:\/\/drive\.google\.com\/(file\/d\/[a-zA-Z0-9_-]+|drive\/folders\/[a-zA-Z0-9_-]+)/;
            ...
            <div className='space-y-2'>
              ...
              <p className='text-xs text-muted-foreground'>
                ...
                <code className='rounded bg-muted px-1 text-xs'>
                  ...
                  @gcp-sa-vertex-rag.iam.gserviceaccount.com
                </code>
              </p>
            </div>
