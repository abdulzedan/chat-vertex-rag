    ...
    try:
        ...
        try:
            ...
            processed_doc = await enhanced_processor.process_file(
                file_path=file_path, file_type=file.content_type, filename=file.filename
            )
            logger.info(
                f"[{request_id}] Enhanced processing complete: {processed_doc['chunk_count']} semantic chunks"
            )
            logger.info(
                f"[{request_id}] Metadata extracted: {len(processed_doc['metadata']['sections'])} sections, {len(processed_doc['tables'])} tables"
            )
                ...
                metadata.update(
                    {
                        ...
                        "sections": processed_doc["metadata"]["sections"][
                            :5
                        ],  # Top 5 sections
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to process document: {str(process_error)}",
            )
        ...
        logger.info(
            f"[{request_id}] Upload completed successfully for: {file.filename}"
        )
        ...
        for doc in search_docs:
            documents.append(
                Document(
                    ...
                    create_time=(
                        datetime.fromisoformat(doc["upload_time"])
...
async def clear_all_documents():
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to clear some documents"
            )
...
async def import_from_google_drive(request: GoogleDriveImportRequest):
    ...
    try:
        ...
        if doc_type in ["document", "spreadsheet", "presentation"]:
            ...
            logger.info(
                f"[{request_id}] Using export URL for {doc_type}: {download_url}"
            )
        else:
        ...
        async with httpx.AsyncClient(timeout=60.0, follow_redirects=True) as client:
            ...
            if (
                "accounts.google.com" in str(response.url)
                or response.status_code != HTTP_OK
            ):
                logger.info(
                    f"[{request_id}] Download failed, trying alternative methods..."
                )
            ...
            if "text/html" in content_type_header and len(response.content) > 0:
                ...
                if (
                    "accounts.google.com" in content_preview
                    or "sign in" in content_preview.lower()
                ):
                    raise HTTPException(
                        status_code=400,
            ...
            logger.info(
                f"[{request_id}] Downloaded file: {filename} ({len(content)} bytes, type: {content_type})"
            )
        ...
        try:
            # Process document using enhanced processor
            processed_doc = await enhanced_processor.process_file(
                file_path=temp_file_path, file_type=content_type, filename=filename
            )
