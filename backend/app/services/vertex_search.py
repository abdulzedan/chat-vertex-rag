...
class VertexSearchService:
    ...
    async def search_documents(
        ...
        try:
            ...
            # Enhanced search request with semantic configuration
            # When filtering by specific documents, request more results to get all chunks
            # For many documents selected, need to request significantly more results
            if document_ids and len(document_ids) > 10:
                search_max_results = max_results * 20  # 20x for large document sets
            elif document_ids:
                search_max_results = max_results * 10  # 10x for smaller sets
            else:
                search_max_results = max_results
            
            if document_ids:
                logger.info(f"Filtering to {len(document_ids)} specific documents")
                logger.info(f"Requesting {search_max_results} results from Vertex AI Search")
            ...
            if document_ids:
                ...
                logger.info(f"Search result document IDs: {[r['document_id'] for r in all_results]}")
                
                # Filter chunks by their parent document ID
                def matches_filter(result, requested_ids):
                    # Check if chunk belongs to requested parent document
                    parent_doc_id = result.get('parent_document_id')
                    return parent_doc_id in requested_ids if parent_doc_id else False
                
                filtered_results = [r for r in all_results if matches_filter(r, document_ids)]
                logger.info(f"Filtered {len(all_results)} results to {len(filtered_results)} based on selected documents (chunks only)")
            ...
            for i, result in enumerate(results[:5]):
                ...
                logger.info(f"Retrieved chunk {i}: {result['filename']} - {content_preview}...")
    ...
    async def generate_response(self, query: str, search_results: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            grouped_results = {}
            # Use more results to capture complete tables that may be split across chunks
            max_results_to_use = min(10, len(search_results))
            logger.info(f"Using top {max_results_to_use} search results out of {len(search_results)} total")
            ...
            logger.info(f"Context being sent to Gemini (length: {len(context)} chars):")
...
Please provide a complete answer based on the information in these documents. Include all relevant details, numbers, and context that would be helpful to fully address the question."""
    ...
    async def _create_enhanced_search_request(self, serving_config_path: str, query: str, 
        ...
        """Create enhanced search request with semantic configuration"""
        ...
        request = discoveryengine.SearchRequest(
            ...
            page_size=max_results,  # Use the max_results passed in (already multiplied if needed)
            
            # Enhanced content search specification
            content_search_spec=discoveryengine.SearchRequest.ContentSearchSpec(
                snippet_spec=discoveryengine.SearchRequest.ContentSearchSpec.SnippetSpec(
                    return_snippet=True,
                    max_snippet_count=5,  # Increased from 3
                    reference_only=False
                ),
                summary_spec=discoveryengine.SearchRequest.ContentSearchSpec.SummarySpec(
                    summary_result_count=5,  # Increased from 3
    ...
    async def _enhance_query(self, query: str) -> str:
        ...
        if 'base' in query_lower and ('offer' in query_lower or 'price' in query_lower or 'rate' in query_lower):
            enhanced_terms.extend(['base pricing', 'baseline', 'base rate', 'starting rate'])
        
        if 'discount' in query_lower or 'off' in query_lower:
            enhanced_terms.extend(['bps off', 'basis points off', 'reduction', 'discount'])
        
        if 'rate' in query_lower:
            enhanced_terms.extend(['percentage', '%', 'bps', 'basis points'])
        
        if 'pricing' in query_lower:
            enhanced_terms.extend(['rates', 'cost', 'fee', 'charge'])
        
        # Combine original query with enhanced terms
        if enhanced_terms:
            return f"{query} {' '.join(enhanced_terms[:3])}"  # Add top 3 most relevant terms
