...
async def upload_document(
    file_path: str, display_name: str, description: Optional[str] = None
) -> rag.RagFile:
    ...
    try:
        ...
        if file_size < 10 * 1024 * 1024:  # Files under 10MB - try direct upload first
            logger.info(
                f"Small file ({file_size} bytes), trying direct upload_file first..."
            )
            try:
                ...
                logger.info(
                    f"Direct upload completed! File: {display_name} with ID: {rag_file.name}"
                )
                return rag_file

            except asyncio.TimeoutError:
                logger.warning(
                    "Direct upload timed out after 45s, falling back to GCS method..."
                )
            except Exception as e:
                logger.warning(
                    f"Direct upload failed: {e}, falling back to GCS method..."
                )
        ...
        try:
            ...
            class ImportingRagFile:
                def __init__(self, _name, display_name):
            ...
            if "There are other operations running" in str(e):
                logger.warning(
                    "Another import operation is already running. Waiting for it to complete..."
                )
                ...
                for attempt in range(max_attempts):
                    ...
                    logger.info(
                        f"Checking if file has been imported (attempt {attempt + 1}/{max_attempts})..."
                    )
                    ...
                    for file in files:
                        if display_name in file.display_name:
                            logger.info(
                                f"File was imported by concurrent operation: {file.name}"
                            )
                ...
                logger.warning(
                    "File not found after waiting. The import operation may have failed."
                )
            ...
            if os.path.getsize(file_path) < 25 * 1024 * 1024:  # Under 25MB limit
                ...
                return await _upload_small_file(
                    file_path, display_name, description, corpus
                )
            else:
                raise
...
async def import_from_drive(drive_url: str) -> str:
    ...
    try:
        ...
        def drive_import():
            try:
                logger.info("Calling import_files with:")
                ...
                if "There are other operations running" in str(e):
                    logger.info(
                        "Concurrent operation detected - another import is already running"
                    )
                    # Extract operation ID from error message
                    import re
                    ...
                    raise RuntimeError(
                        "Another import operation is already running. Please wait for it to complete before trying again."
                    )
                else:
                    import traceback
        ...
        try:
            ...
            if hasattr(import_response, "imported_rag_files_count"):
                logger.info(
                    f"Imported {import_response.imported_rag_files_count} files from Drive"
                )
            if hasattr(import_response, "skipped_rag_files_count"):
                logger.info(
                    f"Skipped {import_response.skipped_rag_files_count} files (already exist)"
                )
            ...
            logger.warning(
                "Drive import operation timed out after 2 minutes. It may still be processing in background."
            )
...
async def _upload_small_file(
    file_path: str, display_name: str, description: Optional[str], corpus
) -> rag.RagFile:
    ...
    try:
        ...
        )  # 30 second timeout for small files
        logger.info(
            f"Direct upload completed! File: {display_name} with ID: {rag_file.name}"
        )
        return rag_file
...
async def query_documents(
    question: str, similarity_top_k: int = 10, vector_distance_threshold: float = 0.5
    ...
    try:
        ...
        if len(files) == 0:
            logger.warning(
                "No files found in RAG corpus. Documents may still be importing."
            )
