...
class VertexAIGroundingService:
        ...
        try:
            ...
            for i, fact in enumerate(facts):
                grounding_fact = discoveryengine.GroundingFact(
                    fact_text=fact[:10000],  # Limit fact length
                    attributes={
                        "id": str(i),
            ...
            # Process response
            result = {
                'support_score': response.support_score if hasattr(response, 'support_score') else 0.0,
                'cited_chunks': [],
                'claims': [],
            ...
            if hasattr(response, 'claims'):
                for claim in response.claims:
                    claim_info = {
                        ...
                        'start_pos': claim.start_pos if hasattr(claim, 'start_pos') else 0,
                        'end_pos': claim.end_pos if hasattr(claim, 'end_pos') else 0,
                        'citation_indices': list(claim.citation_indices) if hasattr(claim, 'citation_indices') else []
                    }
            ...
            logger.info(f"Grounding check complete. Support score: {result['support_score']}")
    ...
    def _add_citations_to_answer(self, answer: str, claims: List[Dict[str, Any]]) -> str:
