...
class DocumentAIProcessor:
    ...
    def __init__(self):
        ...
        self.processor_version = os.getenv("DOCAI_PROCESSOR_VERSION", "pretrained-form-parser-v2.0-2023-08-22")
        ...
        try:
            ...
            for processor in processors:
                if processor.type_ == "FORM_PARSER_PROCESSOR":
                    logger.info(f"Using existing Form Parser processor: {processor.name}")
            ...
            processor = documentai.Processor(
                display_name="Form Parser for RAG",
    ...
    async def process_document_online(
        ...
        try:
            ...
            # Debug logging
            logger.info(f"Document AI response - text length: {len(document.text) if document.text else 0}")
            logger.info(f"Document AI response - pages: {len(document.pages)}")
            logger.info(f"Document AI response - has chunked_document: {hasattr(document, 'chunked_document') and document.chunked_document is not None}")
            if hasattr(document, 'chunked_document') and document.chunked_document:
                logger.info(f"Document AI response - chunks: {len(document.chunked_document.chunks) if document.chunked_document.chunks else 0}")
    ...
    def _extract_structured_data(self, document: documentai.Document, filename: str) -> Dict[str, Any]:
    ...
    def _extract_entities(self, document: documentai.Document) -> Dict[str, List[str]]:
        ...
        try:
            for entity in document.entities:
                ...
                entity_text = entity.text_anchor and self._get_text_from_anchor(
                    entity.text_anchor, document.text
    ...
    def _extract_tables(self, document: documentai.Document, full_text: str) -> List[Dict[str, Any]]:
                ...
                for table_idx, table in enumerate(page.tables):
                    ...
                    for row in table.header_rows:
                        ...
                        for cell in row.cells:
                            cell_text = self._get_text_from_anchor(cell.layout.text_anchor, full_text)
                    ...
                    logger.info(f"Extracted table with {len(table_data['headers'])} headers and {len(table_data['rows'])} rows")
    ...
    def _detect_language(self, text: str) -> str:
        ...
        english_indicators = ['the', 'and', 'of', 'to', 'in', 'a', 'is', 'it', 'you', 'that']
    ...
    def _extract_confidence_scores(self, document: documentai.Document) -> Dict[str, float]:
